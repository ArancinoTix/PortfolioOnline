...
{
    public class Vector4ControlMixer : TypedMotionControlMixer<Vector4>
    {
        public override void ProcessFrame(Playable playable, FrameData info, object playerData)
        {
            ...
            if (component != null && parameterName != null)
            {
                if (!firstFrameHappened)
                {
                    ...
                    {
                        defaultValue = (Vector4)pInfo.GetValue(component);
                    }
        ...
        {
            return new Vector4(behaviour.xEnabled ? Mathf.Lerp(behaviour.startAt.x, behaviour.endAt.x, behaviour.curveX.Evaluate(normalizedTime)) : defaultValue.x,
                               ...
                               behaviour.zEnabled ? Mathf.Lerp(behaviour.startAt.z, behaviour.endAt.z, behaviour.curveZ.Evaluate(normalizedTime)) : defaultValue.z,
                               behaviour.wEnabled ? Mathf.Lerp(behaviour.startAt.w, behaviour.endAt.w, behaviour.curveW.Evaluate(normalizedTime)) : defaultValue.w);
        }
