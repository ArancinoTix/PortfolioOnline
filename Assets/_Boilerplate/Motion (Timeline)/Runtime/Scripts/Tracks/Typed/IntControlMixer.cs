...
{
    public class IntControlMixer : TypedMotionControlMixer<int>
    {
        public override void ProcessFrame(Playable playable, FrameData info, object playerData)
        {
            ...
            if (component != null && parameterName != null)
            {
                if (!firstFrameHappened)
                {
                    ...
                    {
                        defaultValue = (int)pInfo.GetValue(component);
                    }
                ...
                for (int I = 0; I < playable.GetInputCount(); I++)
                {
                    ...
                    ScriptPlayable<TypedControlBehaviour<int>> inputPlayable = (ScriptPlayable<TypedControlBehaviour<int>>)playable.GetInput(I);
                    TypedControlBehaviour<int> behaviour = inputPlayable.GetBehaviour();
                    ...
                    {
                        blendedValue += (int)(GetValue(behaviour, (float)(inputPlayable.GetTime() / inputPlayable.GetDuration())) * inputWeight);
        ...
        private int GetValue(TypedControlBehaviour<int> behaviour, float normalizedTime)
        {
            return (int)Mathf.Lerp(behaviour.startAt, behaviour.endAt, behaviour.curve.Evaluate(normalizedTime));
        }
            ...
            if (director != null && director.playableAsset is TimelineAsset timelineAsset)
            {
                foreach (var track in timelineAsset.GetOutputTracks())
                {
                    ...
                    if (trackBinding == component)
                    {
                        ...
                        {
                            var asset = clip.asset as IntClip;
                            if (asset != null)
                            {
