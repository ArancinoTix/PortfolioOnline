...
{
    public class QuaternionControlMixer : TypedMotionControlMixer<Quaternion>
    {
        public override void ProcessFrame(Playable playable, FrameData info, object playerData)
        {
            ...
            if (component != null && parameterName != null)
            {
                if (!firstFrameHappened)
                {
                    ...
                    {
                        defaultValue = (Quaternion)pInfo.GetValue(component);
                    }
                ...
                for (int I = 0; I < playable.GetInputCount(); I++)
                {
                    ...
                    ScriptPlayable<TypedControlBehaviour<Quaternion>> inputPlayable = (ScriptPlayable<TypedControlBehaviour<Quaternion>>)playable.GetInput(I);
                    TypedControlBehaviour<Quaternion> behaviour = inputPlayable.GetBehaviour();
        ...
        private Quaternion GetValue(TypedControlBehaviour<Quaternion> behaviour, float normalizedTime)
        {
            return Quaternion.Lerp(behaviour.startAt, behaviour.endAt, behaviour.curve.Evaluate(normalizedTime));
        }
            ...
            if (director != null && director.playableAsset is TimelineAsset timelineAsset)
            {
                foreach (var track in timelineAsset.GetOutputTracks())
                {
                    ...
                    if (trackBinding == component)
                    {
                        ...
                        {
                            var asset = clip.asset as QuaternionClip;
                            if (asset != null)
                            {
