using System.ComponentModel;
using System.Linq;
using UnityEngine;
using UnityEngine.Playables;
using UnityEngine.Timeline;

namespace U9.Motion.Timeline
{
    public class MoveRectControlMixer : PlayableBehaviour
    {
        private Vector3 defaultPosition;
        private Vector3 blendedPosition;
        private RectTransform rectTransform;
        private bool firstFrameHappened;

        public override void ProcessFrame(Playable playable, FrameData info, object playerData)
        {
            rectTransform = playerData as RectTransform;

            if (rectTransform != null)
            {
                if (!firstFrameHappened)
                {
                    defaultPosition = rectTransform.anchoredPosition3D;

                    // Caluclate all the clips on this tracks, Start and End times
                    RecalculateAllClipsStartAndEnd(playable);

                    firstFrameHappened = true;
                }

                blendedPosition = Vector3.zero;

                float blendedWeight = 0.0f;
                bool onATrack = false;

                double time = PlayableExtensions.GetTime(playable);

                for (int I = 0; I < playable.GetInputCount(); I++)
                {
                    float inputWeight = playable.GetInputWeight(I);
                    blendedWeight += inputWeight;

                    ScriptPlayable<Vector3ControlBehaviour> inputPlayable = (ScriptPlayable<Vector3ControlBehaviour>)playable.GetInput(I);
                    Vector3ControlBehaviour behaviour = inputPlayable.GetBehaviour();

                    // If not on on any clip at all yet then use the last clips endAt
                    if (blendedWeight == 0)
                    {
                        // Find out if we are after it
                        if (time >= behaviour.end)
                        {
                            // If told to hold then the value is this unless a clip, that is in play, comes along
                            if (behaviour.clipExtrapolation == ClipExtrapolation.Hold)
                                rectTransform.anchoredPosition3D = GetValue(behaviour, 1.0f);
                            else if (behaviour.clipExtrapolation == ClipExtrapolation.Default)
                                rectTransform.anchoredPosition3D = defaultPosition;
                        }
                        // If we are before the first track then use default value
                        else if (I == 0)
                            rectTransform.anchoredPosition3D = defaultPosition;

                        // Go to next input clip
                        continue;
                    }
                    else
                    {
                        blendedPosition += GetValue(behaviour, (float)(inputPlayable.GetTime() / inputPlayable.GetDuration())) * inputWeight;

                        // We are on at least one clip, so we will use the blended value (in case there are multiple clips at this point on the track)
                        onATrack = true;
                    }
                }

                if (onATrack)
                    rectTransform.anchoredPosition3D = blendedPosition;
            }
        }

        public override void OnPlayableDestroy(Playable playable)
        {
            firstFrameHappened = false;

            if (rectTransform != null)
            {
                rectTransform.anchoredPosition3D = defaultPosition;
            }
        }

        private Vector3 GetValue(Vector3ControlBehaviour behaviour, float normalizedTime)
        {
            if(behaviour.useRelative)
                return new Vector3(behaviour.xEnabled ? Mathf.Lerp(behaviour.startAt.x, behaviour.endAt.x, behaviour.curveX.Evaluate(normalizedTime)) + defaultPosition.x: defaultPosition.x,
                                   behaviour.yEnabled ? Mathf.Lerp(behaviour.startAt.y, behaviour.endAt.y, behaviour.curveY.Evaluate(normalizedTime)) + defaultPosition.y: defaultPosition.y,
                                   behaviour.zEnabled ? Mathf.Lerp(behaviour.startAt.z, behaviour.endAt.z, behaviour.curveZ.Evaluate(normalizedTime)) + defaultPosition.z: defaultPosition.z);

            return new Vector3(behaviour.xEnabled ? Mathf.Lerp(behaviour.startAt.x, behaviour.endAt.x, behaviour.curveX.Evaluate(normalizedTime)) : defaultPosition.x,
                               behaviour.yEnabled ? Mathf.Lerp(behaviour.startAt.y, behaviour.endAt.y, behaviour.curveY.Evaluate(normalizedTime)) : defaultPosition.y,
                               behaviour.zEnabled ? Mathf.Lerp(behaviour.startAt.z, behaviour.endAt.z, behaviour.curveZ.Evaluate(normalizedTime)) : defaultPosition.z);
        }

        private void RecalculateAllClipsStartAndEnd(Playable playable)
        {
            var director = playable.GetGraph().GetResolver() as PlayableDirector;
            if (director != null && director.playableAsset is TimelineAsset timelineAsset)
            {
                foreach (var track in timelineAsset.GetOutputTracks())
                {
                    var trackBinding = director.GetGenericBinding(track) as RectTransform;
                    if (trackBinding == rectTransform)
                    {
                        foreach (var clip in track.GetClips())
                        {
                            var asset = clip.asset as Vector3Clip;
                            if (asset != null)
                            {
                                asset.values.start = clip.start;
                                asset.values.end = clip.end;
                            }
                        }
                    }
                }
            }
        }
    }
}