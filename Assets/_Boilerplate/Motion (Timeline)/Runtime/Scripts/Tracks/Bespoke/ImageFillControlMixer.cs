...
{
    public class ImageFillControlMixer : PlayableBehaviour
    {
                    ...
                    if (blendedWeight == 0)
                    {
                        ...
                        if (time >= behaviour.end)
                        {
                            ...
                            if (behaviour.clipExtrapolation == ClipExtrapolation.Hold)
                                image.fillAmount = GetValue(behaviour, 1.0f);
                            else if (behaviour.clipExtrapolation == ClipExtrapolation.Default)
                                image.fillAmount = defaultValue;
                        }
