...
{
    [TrackColor(1.0f, 0.4f, 0.4f)]
    [TrackBindingType(typeof(Material))]
    [TrackClipType(typeof(Vector4Clip))]
    public class MaterialVectorControlTrack : TrackAsset
    {
        [Tooltip("The parameter the clips in this track should manipulate")]
        [SerializeField] private string m_ParameterName;

        MaterialVectorControlMixer m_Mixer;

        public override Playable CreateTrackMixer(PlayableGraph graph, GameObject go, int inputCount)
        {
            var mixer = ScriptPlayable<MaterialVectorControlMixer>.Create(graph, inputCount);
        ...
        {
            clip.displayName = "Material (Vector)";
